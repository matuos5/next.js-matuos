...
export async function GET(req) {
  try {
    ...
    // STEP 2 — محاولات متعددة لجلب الملفات (assets)

    // احتمال 1: GET مع brand_id
    const candidates = [
      {
        desc: "GET brand_id",
      ...
      return NextResponse.json(
        {
          owner: "MATUOS-3MK",
          code: 500,
          msg: "فشل جلب ملفات اللوجوهات من Zoviz (كل المحاولات فشلت)",
          debug: lastAttemptInfo,
        },
        { status: 500 }
      );
    }

    const files = assetsData?.files || assetsData?.result?.files || [];
      ...
      return NextResponse.json(
        {
          owner: "MATUOS-3MK",
          code: 404,
          msg: "لا توجد لوجوهات متوفرة لهذا البراند",
