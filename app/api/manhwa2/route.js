      ...
      return NextResponse.json(
        {
          ...
          examples: [
            "/api/manhwa2?id=SL",
            "/api/manhwa2?id=solo-farming-in-the-tower&all=true"
          ]
        },
        { status: 400 }
      );
  ...
  for (let attempt = 0; attempt < retries; attempt++) {
    ...
    for (const getProxyUrl of shuffledProxies) {
      try {
        ...
        // تجاهل الخطأ وجرب البروكسي التالي (لا حاجة لمتغير الخطأ)
        continue;
      }
    }
    ...
    for (const match of chapterMatches) {
      if (match[1]) {
        try {
          ...
          // إذا فشل تحليل JSON، نجرب regex مباشرة (لا حاجة لمتغير الخطأ)
  ...
  for (let i = 1; i <= Math.min(50, maxChapterToCheck); i++) {
    for (const pattern of patterns) {
      ...
      try {
        ...
        // تجاهل الخطأ (لا حاجة لمتغير الخطأ)
      }
    }
  }
...
async function searchAdditionalChapterPages(mangaId) {
  ...
  for (const pageUrl of additionalPages) {
    try {
      ...
      // تجاهل الصفحات التي لا تعمل (لا حاجة لمتغير الخطأ)
    }
  }
